...
package 
{
    ...
    public class ModAPI 
    {
         ...
         * 
         * @param characterUID the Character's UID
         ...
         * 
         * @example ModAPI.stopTime(this.self.getUID(), 30 * 5, 3, 1)
         */
        public static function stopTime(characterUID:int, length:int = -1, buffer:int = 0, priority:int = int.MAX_VALUE - 1, bypassOptions:Object = null):void
        {
            if (!isReady())
            {
                return;
            }
            m_api.stopTime(characterUID, length, buffer, priority, bypassOptions);
        }

        /**
         * Remove the user's time stop
         * 
         * @param characterUID the Character's UID who triggered time stop  
         */
        public static function resumeTime(characterUID:int):void
        {
            if (!isReady())
            {
                return;
            }
            m_api.resumeTime(characterUID);
        }
         ...
         * 
         */
        public static function applyTimeFreeze(characterUID:int, length:int = -1):void
        {
            if (!isReady())
            {
                return;
            }
            m_api.applyTimeFreeze(characterUID, length);
        }

        /**
         * Unfreeze the character based on UID
         */
        public static function removeTimeFreeze(characterUID:int):void
        {
            if (!isReady())
            {
                return;
            }
            m_api.removeTimeFreeze(characterUID);
        }
    }
}//package
